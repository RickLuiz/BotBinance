...
def get_wallet_assets(min_balance=0.0):
    ...
    try:
        ...
        for balance in balances:
            asset = balance['asset']
            free_balance = float(balance['free'])
            if asset not in ["BRL"] and free_balance > min_balance:
...
def get_top_volatile_cryptos(limit=int(config['limite_criptos']), 
    ...
    try:
        ...
        filtered_tickers = [
            ...
            if ticker['symbol'] in active_symbols and ticker['symbol'] not in blacklist_criptos
        ]
        
        # Obter os ativos da carteira
        wallet_assets = get_wallet_assets(min_balance=1)
...
def buy_crypto(symbol, quantity):
    try:
        ...
        if total_value > saldo_usdt_disponivel:
            print(f"Erro: O valor total da compra ({total_value:.6f} USDT) excede o saldo disponível ({saldo_usdt_disponivel:.6f} USDT).")
            return None
...
def monitor_positions_from_wallet(trailing_stop_percentage=30.0, activation_threshold=30.0, min_stop_loss_percentage=7.0):
    ...
    # Itera sobre cada ativo na carteira    
        for asset, amount in wallet_assets.items():
            ...
            print(f"- Preço atual: {price} USDT")
            ...
            if price > last_price2:
                ...
                print(f"- Atualizando maior preço para: {price} USDT")
            ...
            if trailing_activated.get(symbol, False):
                ...
                if stop_loss < purchase_price + ((purchase_price * 7)/100):
                    ...
                    print(f"  - Stop loss ajustado para: {stop_loss} USDT.")                   

        
            else:
                    print("- PNL não atingiu o limite para ativação do trailing stop.")
            print(f"Valor stop loss: {stop_loss:.6f} USDT")

            # Exibe o valor do stop loss armazenado no dicionário para o ativo atual
            if symbol in stop_loss_data:
                print(f"Stop loss atual para {symbol.replace('USDT', '')}: {stop_loss_data[symbol]:.6f} USDT")
            else:
                print(f"Nenhum stop loss configurado para {symbol.replace('USDT', '')}.")
