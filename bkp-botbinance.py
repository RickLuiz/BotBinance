...
def get_top_volatile_cryptos(limit=int(config['limite_criptos']), hours=int(config['horas_volatilidade']), min_volume=float(config['volume_minimo'].replace(',', '.'))):
    try:
        #print("Obtendo todos os tickers da Binance...")
        ...
        #print(f"Ativos da carteira encontrados: {wallet_assets}")

        for ticker in tickers:
            symbol = ticker['symbol']
            volume = float(ticker['quoteVolume'])
            
            # Verificar se o ticker é par com USDT e atende ao volume mínimo
            if symbol.endswith('USDT') and volume >= min_volume:
                asset = symbol.replace('USDT', '')  # Remove 'USDT' para obter o ativo

                # Verificar se a cripto não está na carteira com saldo >= 0.1 unidade
                if asset not in wallet_assets or wallet_assets[asset] < 0.1:
                    ...
                    if volatility is not None:
                        ...
                        if get_moving_averages(symbol) and calculate_rsi(symbol) < 70:
                            volatility_data.append({'symbol': symbol, 'volatility': volatility, 'volume': volume})
                        else:
                            # Caso não atenda aos critérios, apenas continue (sem necessidade de indentação extra)
                            pass
        
        # Ordenar os ativos por volatilidade e retornar os top N
        top_volatile = sorted(volatility_data, key=lambda x: x['volatility'], reverse=True)
        #print(f"Top voláteis calculados: {top_volatile}")
...
def main():
    ...
    while True:
        ...
        if config['on_off']:  # Verifica se o bot está ativado
            ...
            # Mostrar o top de voláteis no log
            ...
            for i, crypto in enumerate(top_cryptos, start=1):
                ...
                print(f"{i}. {symbol_without_usdt} - Volatilidade: {volatility_percentage:.2f}%")
