...
def calculate_bollinger_bands(symbol, interval='1d', days=20):
    """
...
# Calcula o RSI
def calculate_rsi(symbol, interval='1d', days=14):
    """
    Calcula o Índice de Força Relativa (RSI) para um ativo em um intervalo específico.

    :param symbol: Símbolo da criptomoeda, e.g., 'BTCUSDT'.
    :param interval: Intervalo das velas (e.g., '1h', '4h'). Default é '1h'.
    :param days: Número de horas para análise. Default é 14.
...
def get_moving_averages(symbol, interval='1d', days=20):
    """
    Calcula a média móvel simples (SMA) para um ativo em um intervalo específico.

    :param symbol: Símbolo da criptomoeda, e.g., 'BTCUSDT'.
    :param interval: Intervalo das velas (e.g., '1h', '4h'). Default é '1h'.
    :param days: Número de horas para análise. Default é 20.
...
def process_ticker(ticker, min_volume, wallet_assets, days):
    try:
        ...
        if symbol.endswith('USDT') and volume >= min_volume:
            ...
            if asset not in wallet_assets or wallet_assets[asset] < 0.1:
                ...
                volatility = get_historical_volatility(symbol, days=days)
...
def get_top_volatile_cryptos(limit=int(config['limite_criptos']), days=int(config['dias_volatilidade']), min_volume=float(config['volume_minimo'].replace(',', '.'))):
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(process_ticker, ticker, min_volume, wallet_assets, days) for ticker in tickers]
...
def get_historical_volatility(symbol, days):
    try:
        ...
        klines = client.get_historical_klines(symbol, Client.KLINE_INTERVAL_1DAY, f"{days} day ago UTC")
...
def buy_crypto(symbol, quantity):
    try:
        ...
        # Se o filtro MIN_NOTIONAL não for encontrado, o código continua sem impedir a compra
       # if min_notional is None:
        ...
        if min_notional and total_value < min_notional:
            message = (f"Erro: Valor total da compra ({total_value:.2f} USDT) é menor que o mínimo permitido ({min_notional:.2f} USDT) para {symbol.replace('USDT', '')}.")
            # send_email("Erro na Compra", message)
            print(message)
            return None
...
# Função monitor_positions_from_wallet
def monitor_positions_from_wallet(trailing_stop_percentage=20.0, activation_threshold=30.0):
    """
    Monitora as posições na carteira e realiza vendas baseadas em trailing stop.
    
    :param trailing_stop_percentage: Percentual de proteção do lucro após atingir a ativação.
    :param activation_threshold: Percentual de valorização para ativar o trailing stop.
    """
    try:
        ...
        for asset, amount in wallet_assets.items():
            ...
            if purchase_price > 0:
                try:
                    # Calcula lucro e percentual
                    pnl = (price - purchase_price) * amount
                    ...
                    # Implementação do trailing stop
                    if pnl_percent >= activation_threshold:
                        # Ativa trailing stop
                        trailing_stop_price = purchase_price * (1 + (activation_threshold / 100))  # Preço de ativação
                        stop_loss_price = price * (1 - (trailing_stop_percentage / 100))          # Stop Loss dinâmico

                        print(f"- Trailing Stop ativado! Preço de ativação: {trailing_stop_price:.2f} USDT")
                        print(f"- Stop dinâmico ajustado para: {stop_loss_price:.2f} USDT")

                        if price <= stop_loss_price:
                            print(f"Preço caiu para o stop dinâmico de {stop_loss_price:.2f} USDT. Vendendo {asset}.")
                            sell_crypto(symbol, adjusted_quantity)
                    elif pnl_percent >= float(config['lucro_venda'].replace(',', '.')):
                        # Venda normal se atingir a meta de lucro
                        print(f"Lucro de {pnl_percent:.2f}% atingido para {symbol.replace('USDT', '')}. Vendendo {asset}.")
                        sell_crypto(symbol, adjusted_quantity)

                except Exception as e:
                    print(f"Erro ao calcular o PNL ou trailing stop para {symbol}: {e}")
            else:
                print(f"Preço de compra inválido para {symbol.replace('USDT', '')}. Ignorando.")
...
def main():
    ...
    #x = 1 / 0  # Isso causará um ZeroDivisionError
    while True:
        global config
        ...
        DIAS_VOLATILIDADE = int(config['dias_volatilidade'])  # Se for inteiro, converte para int
        ...
        if config['on_off']:  # Verifica se o bot está ativado
            ...
            print(f"Periodo de análise de volatilidade: {DIAS_VOLATILIDADE} dia(s) ")
            message = f"Periodo de análise de volatilidade: {DIAS_VOLATILIDADE} hrs "
            ...
            top_cryptos = get_top_volatile_cryptos(
                ...
                days=DIAS_VOLATILIDADE,  # Passando o valor de DIAS_VOLATILIDADE
